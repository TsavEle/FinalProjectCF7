...
@WebServlet("/saveLawyer")
public class saveLawyerController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            if (lawyerDAO.getByEmail(email) != null) {
                req.getSession().setAttribute("emailError", "Email already exists!");
                resp.sendRedirect(req.getContextPath() + "/insertLawyer");
                return;
            }

            if (lawyerDAO.getByVat(vat) != null) {
                req.getSession().setAttribute("vatError", "Vat already exists!");
                resp.sendRedirect(req.getContextPath() + "/insertLawyer");
                return;
            }
            lawyerDAO.insert(lawyer, selectedSkillIds);
            // Redirect to a success page or back to the form
            req.getSession().setAttribute("successMessage", "Lawyer added successfully!");
